import numpy as np
import math
from ..miniworld import MiniWorldEnv, DiscreteMiniworldEnv, Room
...
class DiscreteOneRoom(DiscreteMiniworldEnv):
    """
    Environment in which the goal is to go to a red box
    placed randomly in one big room.
    """
    ...
    def _gen_world(self,*args,**kwargs):
        room = self.add_rect_room(
            min_x=0,
            max_x=self.size,
            min_z=0,
...
class DiscreteOneRoomS6(DiscreteOneRoom):
    def __init__(self, max_episode_steps=100, **kwargs):
        super().__init__(size=6, max_episode_steps=max_episode_steps, **kwargs)

class DiscreteOneRoomS6Fast(DiscreteOneRoomS6):
