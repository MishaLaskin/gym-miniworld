...
from ..miniworld import ContinuousMiniworldEnv, MiniWorldEnv, DiscreteMiniworldEnv, Room
...
class DiscreteFourRooms(DiscreteMiniworldEnv):
    """
    Classic four rooms environment.
    The agent must reach the red box to get a reward.
    """
    ...
    def _gen_world(self, prob_constraint, min_goal_dist, max_goal_dist):
        ...
        if np.random.random() < prob_constraint:
            self.box = self.place_entity_nearby(Box(
                color='red', invisible=True), self.agent.pos, min_goal_dist, max_goal_dist, share_room=True)
        else:
            self.box = self.place_entity(Box(color='red', invisible=True))
